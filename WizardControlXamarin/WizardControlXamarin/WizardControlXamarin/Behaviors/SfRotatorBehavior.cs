...
using WizardControlXamarin.ViewModel;
using Xamarin.Forms;
using Xamarin.Forms.Internals;

namespace WizardControlXamarin.Behavior
{
    /// <summary>
    ...
    public class SfRotatorBehavior : Behavior<SfRotator>
    {
        ...
        public void Animation(SfRotator rotator, double selectedIndex)
        {
            if ( rotator != null && rotator.ItemsSource != null && rotator.ItemsSource.Count() > 0 )
            {
                ...
                var viewModel = rotator.BindingContext as AnimationViewModel;
                ...
                if ( Device.RuntimePlatform != Device.UWP )
                {
                    ...
                    var childElement = ( ( ( currentItem as WizardPageModel).RotatorItem as ContentView ).Children[0] as StackLayout ).Children.ToList();
                    if ( childElement != null && childElement.Count > 0 )
                    {
                        this.StartAnimation(childElement, currentItem as WizardPageModel);
                    }
                    ...
                    if ( index != this.previousIndex )
                    {
                        ...
                        var previousChildElement = ( ( ( previousItem as WizardPageModel).RotatorItem as ContentView ).Children[0] as StackLayout ).Children.ToList();
        ...
        public async void StartAnimation(List<View> childElement, WizardPageModel item)
        {
            if (childElement != null && item != null)
            {
                ...
                animation.Commit((item as WizardPageModel).RotatorItem as ContentView, "animation", 16, 500);
